...
export interface JsonRecordEditorLabels {
...
interface JsonRecordEditorProps<T> {
 ...
 */
export function JsonRecordEditor<T>({
  initial,
  schemaHint,
  labels,
  onSubmit,
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-3">
      ...
      />
      {error && (
        <p className="rounded border border-danger-border bg-danger-surface px-2 py-1 text-xs text-danger-foreground">
          {error}
        </p>
      )}
