...
export default function SystemSettingsPage() {
  const t = useTranslations("settings.runtime.config");
  ...
  // Order known categories first (per `CATEGORY_ORDER`), then append
  // any unknowns the backend introduces — so a freshly-added category
  ...
  // Active tab fallback — the persisted `activeTab` may not exist in
  // the current config (backend dropped a category between mounts);
  // pick the first available so the body never renders against an
  // absent key.
  const effectiveTab = categories.includes(activeTab)
    ? activeTab
    : categories[0];
  ...
  return (
    <SettingsPageShell title={t("title")} subtitle={t("description")}>
      ...
      <div className="sticky bottom-0 mt-8 flex items-center justify-end gap-2 border-t border-divider bg-surface-raised px-0 py-4">
        {hasChanges && (
          <span className="me-auto text-xs text-warning-foreground">
    ...
    <div className="flex items-center gap-3 px-6 py-3">
      ...
      <div className="w-36 shrink-0">
        <FormInput
          ...
          className={`text-end text-sm ${isModified ? "border-warning-border" : ""}`}
        />
      </div>
    </div>
  );
