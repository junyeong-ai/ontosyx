...
const KIND_STYLES: Record<string, string> = {
  correction:
    "text-info-foreground bg-info-surface ring-info-foreground dark:text-info-foreground",
  ...
  return (
    <div
      className={cn(
        ...
        isExpanded
          ? "border-brand-border bg-surface-base shadow-sm"
      ...
      <button onClick={onToggle} className="flex w-full items-center gap-3 px-4 py-3 text-left">
        ...
        <span className="hidden shrink-0 items-center gap-1 sm:flex">
          ...
          {entry.affected_labels.length > 3 && (
            <span className="text-2xs text-muted-foreground">+{entry.affected_labels.length - 3}</span>
          )}
        </span>
