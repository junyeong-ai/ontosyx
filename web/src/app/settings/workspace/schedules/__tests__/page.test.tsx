...
import SchedulesPage from "@/app/settings/workspace/schedules/page";
import {
  listScheduledTasks,
  updateScheduledTask,
  deleteScheduledTask,
...
describe("SchedulesPage", () => {
  ...
  it("delete with confirm=true reverts optimistic remove if the API fails", async () => {
    // The mutation pipeline is: optimistic cache delete → mutationFn
    // throws → onError reverts the snapshot → onSettled invalidates
    // and refetches. Use `mockResolvedValue` (not `Once`) so the
    // post-settle refetch returns the same row — the assertion is
    // about the optimistic revert, not the server going out of sync.
    vi.mocked(listScheduledTasks).mockResolvedValue([SAMPLE_TASK]);
    ...
    fireEvent.click(screen.getByRole("button", { name: /^Delete$/ }));
    // After the failure, the error toast fires AND the row stays
    // visible (optimistic delete was reverted).
