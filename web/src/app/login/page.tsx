...
function LoginContent() {
  ...
  // Render a neutral shell while the auth round-trip resolves so we
  // don't flash a sign-in button to a user who's already signed in.
  if (mode.kind !== "unauthenticated") {
    return (
      <main
        id="main"
        className="flex min-h-screen items-center justify-center bg-surface-raised"
      >
  ...
  return (
    <main
      id="main"
      className="flex min-h-screen items-center justify-center bg-surface-raised px-4"
    >
      <div className="w-full max-w-sm">
        <div className="rounded-2xl border border-divider bg-surface-base p-8 shadow-2xl">
          ...
          <a
            ...
            className="mt-6 flex items-center justify-center gap-3 rounded-lg border border-divider bg-surface-base px-6 py-3 text-sm font-medium text-foreground-strong transition-colors hover:bg-surface-inset focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-foreground focus-visible:ring-offset-2 aria-disabled:pointer-events-none aria-disabled:opacity-60"
          >
            {signingIn ? (
              <Spinner size="sm" />
            ) : (
