...
export default function KnowledgePage() {
  ...
  const url = useTableUrlState({ filters: ["status", "kind"] });
  const statusFilter = url.filters.status ?? "";
  const kindFilter = url.filters.kind ?? "";
  ...
  const handleStatus = (id: string, status: KnowledgeStatus) => {
    const previous = entries.find((e) => e.id === id)?.status;
    updateStatus.mutate(
      { id, status },
      {
        onSuccess: () => {
          if (previous && previous !== status) {
            toast.undoable({
              message: t("toast.statusChanged", {
                status: statusLabel(status),
              }),
              undoLabel: t("toast.undo"),
              onUndo: () =>
                updateStatus.mutate({ id, status: previous }),
            });
          } else {
            toast.success(
              t("toast.statusChanged", { status: statusLabel(status) }),
            );
          }
        },
  ...
  usePublishModeCount("knowledgeBase", staleCount, "warning");
      ...
      filters={
        <>
          <FormSelect
            ...
            value={statusFilter}
            onChange={(e) => url.setFilter("status", e.target.value || null)}
            className="w-auto"
          >
          ...
          <FormSelect
            ...
            value={kindFilter}
            onChange={(e) => url.setFilter("kind", e.target.value || null)}
            className="w-auto"
          >
