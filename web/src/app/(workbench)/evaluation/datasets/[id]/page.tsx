...
export default function EvaluationDatasetDetailPage({
    ...
    createRun.mutate(
      ...
      },
      {
        onSuccess: (res) => {
          ...
          router.push(
            `/evaluation/${encodeURIComponent(res.run.id)}`,
          );
        },
        onError: (err) => {
          toast.error(
  ...
  if (!isAdmin) {
    return (
      <WorkbenchPageShell title={t("detail.title")}>
    ...
    <WorkbenchPageShell
      ...
      eyebrow={
        <Link
          ...
          className="inline-flex items-center gap-1 rounded transition-colors duration-[var(--duration-quick)] ease-[var(--ease-out)] hover:text-foreground-strong focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-foreground/40"
        >
          <ArrowLeft className="h-3 w-3" aria-hidden />
          {t("title")}
        </Link>
      }
      headline={dataset?.name ?? t("detail.title")}
      pageState={pageState}
    >
      <PageStateView
        state={pageState}
