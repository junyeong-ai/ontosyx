...
export function ProvenanceAuditFacet() {
  ...
  const url = useTableUrlState({ filters: ["activity", "agent"] });
  const activityFilter = url.filters.activity ?? "all";
  const agentFilter = url.filters.agent ?? "all";
      ...
      {hasOntology && (
        <>
          {trail.isLoading ? (
            ...
            <EmptyState
              kind={isFilterActive(filter) ? "no-results" : "no-data"}
              title={
              ...
              {trail.hasNextPage && (
                <div className="flex justify-center pt-2">
                  <button
                    ...
                    className="rounded-md border border-divider bg-surface-base px-3 py-1.5 text-xs font-medium text-foreground hover:bg-surface-raised disabled:cursor-not-allowed disabled:opacity-50"
                  >
