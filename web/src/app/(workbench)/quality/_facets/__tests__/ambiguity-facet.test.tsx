...
import { AmbiguityFacet } from "@/app/(workbench)/quality/_facets/ambiguity-facet";
import {
...
describe("AmbiguityFacet", () => {
  ...
  it("classifies summaries into pending / stale / resolved status counts", async () => {
    ...
    expect(optionLabels.join(" ")).toMatch(/Needs attention.*\(1\)/);
    expect(optionLabels.join(" ")).toMatch(/Re-check.*\(1\)/);
    expect(optionLabels.join(" ")).toMatch(/Resolved.*\(1\)/);
  });
  ...
  it("Revoke button on a resolved row calls the API", async () => {
    ...
    await waitFor(() =>
      expect(toast.undoable).toHaveBeenCalledWith(
        expect.objectContaining({ message: "Resolution revoked" }),
      ),
    );
  });
