...
import { StaleFacet } from "@/app/(workbench)/quality/_facets/stale-facet";
import {
  listStaleProposals,
  decideStaleProposal,
  listTypeCandidates,
...
describe("StaleFacet", () => {
  ...
  it("renders pending rows by default + decided cohort is reachable via the status filter", async () => {
    vi.mocked(listStaleProposals).mockResolvedValue([PENDING, APPROVED]);
    renderPage();
    ...
    await waitFor(() =>
      expect(screen.getByText("OldEdge")).toBeInTheDocument(),
    );
