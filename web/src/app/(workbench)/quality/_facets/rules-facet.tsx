...
import { useChromeActions } from "@/components/workbench/workbench-page-shell";
...
export function RulesFacet() {
  ...
  // Hook MUST sit above any early return — rules of hooks. Its
  // returned portal node is rendered (or absent) by the consumer
  // below; the hook itself runs unconditionally on every render.
  const chromeActions = useChromeActions(
    !loading && !errored && !formOpen ? (
      <>
        <Button
          variant="outline"
          size="sm"
  ...
  return (
    <div className="flex flex-col gap-4">
      {chromeActions}
      {dashboard.length > 0 && (
        <KpiStrip passing={passed} failing={failed} pending={pending} t={t} />
      )}
      ...
      {dashboard.length === 0 && !formOpen ? (
        ...
        />
      ) : dashboard.length === 0 && formOpen ? null : (
      <div className="mt-2 overflow-x-auto -mx-6 px-6" tabIndex={0} role="region" aria-label={tCommon("scrollableTableAria")}>
...
function KpiStrip({
  ...
  const cells: Array<{
    ...
    { tone: "success", label: t("summary.passing"), value: passing },
    { tone: "danger", label: t("summary.failing"), value: failing },
    { tone: "neutral", label: t("summary.pending"), value: pending },
  ];
  const dotClass = {
  ...
  return (
    <div className="flex items-center gap-4 rounded-md border border-divider bg-surface-base px-4 py-2">
      {cells.map((cell, idx) => (
        <div key={cell.label} className="flex items-center gap-2 text-xs">
          ...
          <span
            className={cn("h-1.5 w-1.5 shrink-0 rounded-full", dotClass[cell.tone])}
            aria-hidden
          />
          <span className="font-semibold tabular-nums text-foreground-strong">
            {cell.value}
          </span>
          <span className="text-foreground-muted">{cell.label}</span>
        </div>
      ))}
    </div>
  );
