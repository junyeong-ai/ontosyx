...
impl<'a> ResolverCtx<'a> {
    ...
    pub(super) fn bind_node_variable(&mut self, variable: &str, label: &str) {
            ...
            .find(|n| n.label == *label)
        {
            ...
            // broken row.
            let Ok(variable) = ox_core::variable_name::VariableName::new(variable) else {
                return;
            };
    ...
    pub(super) fn resolve_variable_property(&mut self, variable: &str, property_name: &str) {
        ...
        {
            let variable = ox_core::variable_name::VariableName::new(variable).ok();
            let Ok(property_name) = ox_core::property_key::PropertyKey::new(property_name) else {
                return;
            };
