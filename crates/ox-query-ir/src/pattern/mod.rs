...
impl PatternIR {
    ...
    ///
    /// Fails with [`ox_core::error::OxError::Validation`] when any node
    ...
    pub fn compile(&self) -> ox_core::error::OxResult<QueryIR> {
...
mod tests {
    use super::*;
    use crate::query::{ComparisonOp, LogicalOp};
    ...
    fn pk(s: &str) -> ox_core::property_key::PropertyKey {
        ox_core::property_key::PropertyKey::new(s).expect("test property key literal must be valid")
    }
    ...
    fn decompile_non_match_operation_returns_empty() {
        ...
        use crate::query::{NodeRef, PathAlgorithm};
        let query = QueryIR {
            schema_version: crate::query::QUERY_IR_SCHEMA_VERSION,
