    ...
    match op {
        ...
        QueryOp::Mutate {
            ...
            for op in operations {
                match op {
                    crate::query::MutateOp::CreateNode { label, .. }
                    | crate::query::MutateOp::MergeNode { label, .. } => {
                        labels.push(label.clone());
                    }
                    _ => {}
                }
        ...
        QueryOp::Analytics { source, .. } => match source {
            crate::query::AnalyticsSource::Labels { labels: src_labels } => {
                labels.extend(src_labels.iter().cloned());
            }
            crate::query::AnalyticsSource::Subgraph { filter } => {
...
fn extract_edge_labels_from_op(op: &QueryOp, labels: &mut Vec<GraphLabel>) {
    match op {
        ...
        QueryOp::Mutate {
            ...
            for op in operations {
                match op {
                    crate::query::MutateOp::CreateEdge { label, .. }
                    | crate::query::MutateOp::MergeEdge { label, .. } => {
                        labels.push(label.clone());
                    }
                    _ => {}
                }
        ...
        let qir = QueryIR {
            ...
            operation: QueryOp::Match {
                patterns: vec![GraphPattern::Node {
                    variable: ox_core::variable_name::VariableName::new("c").expect("valid"),
        ...
        let qir = QueryIR {
            ...
            operation: QueryOp::Match {
                patterns: vec![
                    ...
                    GraphPattern::Relationship {
                        variable: Some(
                            ox_core::variable_name::VariableName::new("r").expect("valid"),
                        ),
                        label: Some(gl("PLACED")),
                        source: ox_core::variable_name::VariableName::new("c").expect("valid"),
                        target: ox_core::variable_name::VariableName::new("o").expect("valid"),
                        direction: Direction::Outgoing,
                        property_filters: vec![],
                        var_length: None,
                    },
                    GraphPattern::Node {
                        variable: ox_core::variable_name::VariableName::new("o").expect("valid"),
                        label: Some(gl("Order")),
                        property_filters: vec![],
                    },
                ],
