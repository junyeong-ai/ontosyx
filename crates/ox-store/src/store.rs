...
pub struct AnalysisSnapshot {
    ...
    /// `ox_source::AnalysisScope` accumulated across the project's
...
pub trait InsightStore: Send + Sync {
    ...
    ) -> OxResult<ox_query_ir::insight::InsightDef>;
    ...
    async fn update_insight(
        &self,
        id: &ox_query_ir::insight::InsightId,
    ...
    async fn get_insight(
        &self,
        id: &ox_query_ir::insight::InsightId,
    ) -> OxResult<Option<ox_query_ir::insight::InsightDef>>;
    ...
    /// `concept_anchors` (1-pager: "용어 사전이 다리") and freeform
    /// `tags`. Each filter slot is array-overlap (`&&`) — non-empty
    /// argument means "any of these"; empty means "don't filter on
    /// this axis". Multiple slots AND together.
    async fn list_insights(
        &self,
        filter: &InsightFilter,
        pagination: &CursorParams,
    ) -> OxResult<CursorPage<ox_query_ir::insight::InsightDef>>;
...
pub trait WorkspaceStore: Send + Sync {
    ...
    /// `LanguageTag::parse` syntax); both `admin_locale_fallback`
    /// and `llm_locale_fallback` must be non-empty JSONB arrays of
    /// the same shape. All three are enforced by DB CHECK
    /// constraints.
...
// ---------------------------------------------------------------------------
// EvaluationStore — RAGAS-style metric loop persistence
// ---------------------------------------------------------------------------
...
/// pattern used by `OntologyDraftStore` is intentionally absent —
/// evaluation rows are append-only / UPSERT-on-natural-key, never
/// in-place CAS.
///
/// ## Retention
///
/// Runs are not auto-archived; the dashboard's "compact older
/// than N days" sweep is a future cron that calls
/// `delete_evaluation_runs_older_than`. Until then operators
